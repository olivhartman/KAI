...
- **Status values:** PENDING → RESEARCHING → COMPLETED → ASSIGNED
...
- `J` (Status): SCHEDULED → TRIGGERED → COMPLETED
...
| C | Status | Current workflow stage | DRAFT → SCRIPT_READY → GENERATING → MERGING → READY_TO_PUBLISH → PUBLISHED → ERROR |
...
8. **Comment Posting:**
   ...
   - Update Comment Posted → YES

**Error Handling:**
- Any API failure → Status = ERROR
...
| D | Status | EXTRACTING → READY_TO_POST → POSTING → POSTED → ERROR |
...
4. **Multi-Platform Publishing:**
   - **Instagram:** Blotato API → Update column K
   - **TikTok:** Blotato API → Update column L
   - **Twitter:** Blotato API → Update column M
   - **Telegram:** Telegram Bot API → Update column N
   - **YouTube Shorts:** Direct YouTube API → Update column O
5. **Status Updates:** Each platform column gets:
   - PENDING → POSTING → POSTED (with timestamp) OR ERROR
...
| C | Status | IDEA → GENERATING → READY_TO_POST → POSTED → ERROR |
...
| C | Status | PLANNING → GENERATING → READY_TO_POST → POSTED |
...
- Match required emotion → get character ref URL
...
```
IF topic contains "McLaren wins" → Select WARD003 (McLaren shirt)
IF topic contains "Ferrari pole" → Select WARD002 (Ferrari outfit)
IF no team match → Select WARD005 (Generic F1 fan tee)
```
...
- Query this sheet → pass URLs to video gen API
...
1. In n8n, go to Credentials → Google Sheets OAuth2
...
2. Data → Named ranges → Create "ResearchLog"
3. Repeat for other key ranges

---

## 🛠️ N8N INTEGRATION PATTERNS
...
```
DRAFT → GENERATING → READY_TO_POST → POSTING → POSTED → [ARCHIVED]
                                                ↓
                                              ERROR
```
